...
Layer *new_linear_layer(int input_size, int output_size) {
  ...
  // params
  layer->params = malloc(5 * sizeof(Matrix *));
  ...
  // options
  layer->options = malloc(2 * sizeof(int));
  layer->options[0] = input_size;
  layer->options[1] = output_size;
  layer->num_options = 2;
...
Layer *new_sigmoid_layer(int input_size) {
  ...
  // options
  layer->options = malloc(1 * sizeof(int));
  layer->options[0] = input_size;
  layer->num_options = 1;
